...
public class FoundInfoActivity extends AppCompatActivity {
                ...
                switch (msg.what) {
                    case REFRESH_DATA_FINISH:
                        if (new MyFoundAdapter() != null) {
                            new MyFoundAdapter().notifyDataSetChanged();///下拉刷新完成
                        }
                        lvFound.onRefreshComplete();
                        break;
                    case LOAD_DATA_FINISH:
                        if (new MyFoundAdapter() != null) {
                            new MyFoundAdapter().notifyDataSetChanged();//上啦加载更多完成
                        }
    ...
    protected void onResume() {
        super.onResume();
        if (clickFlag) { //用户点击了某个失物信息条目，将不会影响数据的分页标记
            clickFlag = false;
            return;
        }
        skip = 0;
